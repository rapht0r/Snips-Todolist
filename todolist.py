...
class TodoList:
    ...
    def add_item(self, intentmessage):
        ...
        if added_items:
            ...
            first_str = items_str + random.choice(["{} hinzugefügt".format(word_pl_sg),
                                                   "{} auf die Tuhdu-liste geschrieben".format(word_pl_sg)])
        ...
        if dublicate_items:
            ...
            if len(dublicate_items) >= 2:
                ...
                word_pl_sg = "ist"
            second_str = items_str + random.choice(["{} schon auf der Tuhdu-Liste.".format(word_pl_sg),
                                                    "{} auf der Tuhdu-Liste schon vorhanden.".format(word_pl_sg)])
            response += second_str
        self.save_todolist()
        return response
    ...
    def remove_item(self, intentmessage):
        ...
        if removed_items:
            ...
            first_str = items_str + random.choice(["{} entfernt".format(word_pl_sg),
                                                   "{} von der Tuhdu-liste entfernt".format(word_pl_sg)])
        ...
        if notlist_items:
            ...
            second_str = items_str + random.choice(["{} nicht auf der Liste.".format(word_pl_sg),
                                                    "{} auf der Tuhdu-liste nicht vorhanden.".format(word_pl_sg)])
            response += second_str
        self.save_todolist()
        return response
    ...
    def is_item(self, intentmessage):
        ...
        if item in self.todolist:
            response = "Ja, {item} steht auf der Tuhdu-liste.".format(item=str(item))
        else:
            response = "Nein, {item} ist nicht auf der Tuhdu-liste.".format(item=str(item))
        return response

    def try_clear(self):
        if len(self.todolist) > 1:
            response = "Die Tuhdu-liste enthält noch {num} Elemente." \
                       " Bist du dir sicher?".format(num=len(self.todolist))
        elif len(self.todolist) == 1:
            response = "Die Tuhdu-liste enthält noch ein Element. Bist du dir sicher?"
        else:
    ...
    def clear_confirmed(self, intentmessage):
        if intentmessage.slots.answer.first().value == "yes":
            self.todolist = []
            self.save_todolist()
            return "Die Tuhdu-liste wurde geleert."
        else:
            return "Die Tuhdu-liste wurde nicht geleert."
    ...
    def show(self):
        if len(self.todolist) > 1:
            ...
            response = "Die Tuhdu-liste enthält {items}und {last}.".format(
                items=todolist_str, last=self.todolist[-1])
        elif len(self.todolist) == 1:
            response = "Die Tuhdu-liste enthält nur {item}.".format(item=self.todolist[0])
        else:  # If todolist is empty
            response = "Die Tuhdu-liste ist leer."
        return response

    def send(self):
        if len(self.todolist) == 0:
            return "Die Tuhdu-liste ist leer. Eine Email ist daher überflüssig."
